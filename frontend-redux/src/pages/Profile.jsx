...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        // Fire all microservice requests concurrently!
        const [userRes, favRes, prefRes, histRes] = await Promise.all([
          // Port 8001: User Service
          fetch('http://localhost:8001/api/users/me', { headers }).then(res => res.ok ? res.json() : null),
          fetch('http://localhost:8001/api/favorites/', { headers }).then(res => res.ok ? res.json() : []).catch(() => []),
          fetch('http://localhost:8001/api/users/preferences', { headers }).then(res => res.ok ? res.json() : { preferences: {} }).catch(() => ({ preferences: {} })),
          
          // Port 8003: Review Service
          fetch('http://localhost:8003/api/reviews/me', { headers }).then(res => res.ok ? res.json() : []).catch(() => [])
        ]);
        
        if (userRes) {
          setProfileData(userRes);
          setSettingsForm(userRes);
        }
        ...
        if (prefRes && prefRes.preferences) {
          setPrefsForm(prev => ({ ...prev, ...prefRes.preferences }));
        }
      } catch (error) {
  ...
  const submitSettings = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      // Strip out non-updateable fields before sending
      const { id, _id, email, hashed_password, preferences, profile_picture, ...updatePayload } = settingsForm;
      
      const res = await fetch('http://localhost:8001/api/users/me', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const submitPrefs = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('http://localhost:8001/api/users/preferences', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleFileChange = async (e) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      try {
        const token = localStorage.getItem('token');
        const response = await fetch('http://localhost:8001/api/users/me/photo', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`
          },
        ...
        setMsg({ type: 'error', text: error.message || 'Failed to upload photo.' });
      } finally {
        e.target.value = null; 
      }
    }
  ...
  // Route user photos to Port 8001
  const getImageUrl = (path) => {
    if (!path) return null;
    return `http://localhost:8001${path}`;
  };

  if (authLoading || loading || !profileData) return <div className="p-10 text-center text-gray-500 font-medium">Loading profile...</div>;
      ...
      {favorites.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {favorites.map(restaurant => (
            <RestaurantCard 
              key={restaurant.id || restaurant.restaurant_id || restaurant._id} 
    ...
    <div>
      ...
      {history.length === 0 ? (
        ...
        <div className="space-y-6">
          {history.map(rev => (
            <div key={rev.id || rev._id} className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm flex flex-col hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-3">
                <div>
                  ...
                  <p className="text-sm text-gray-900 mt-2 font-bold text-lg">{rev.restaurant_name || "Restaurant Review"}</p>
                </div>
