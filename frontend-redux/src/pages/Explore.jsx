...
import { fetchRestaurantsStart, fetchRestaurantsSuccess, fetchRestaurantsFailure } from '../store/restaurantSlice';
...
const Explore = () => {
  ...
  useEffect(() => {
    const fetchRestaurants = async () => {
      dispatch(fetchRestaurantsStart()); // Sets Redux loading state to true
      try {
        ...
        // Directly target the Restaurant Service container on PORT 8002!
        const endpoint = `http://localhost:8002/api/restaurants/?${params.toString()}`;
        
        // Use standard fetch to bypass the misconfigured axiosClient
        ...
        dispatch(fetchRestaurantsSuccess(data)); // Saves data to Redux
      } catch (error) {
    ...
    fetchRestaurants();
  }, [searchQuery, locationQuery, dispatch]); // Trigger when either changes
