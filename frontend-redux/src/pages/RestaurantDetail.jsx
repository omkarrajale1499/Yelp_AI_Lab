...
const RestaurantDetail = () => {
  ...
  // Point static photos to the Restaurant Service
  const getImageUrl = (filename) => {
    return `http://localhost:8002/static/restaurant_photos/${filename}`;
  };
  ...
  const getDynamicHours = (restId, dbHours) => {
    const now = new Date();
    const currentDayIndex = now.getDay(); 
    const currentMinutes = now.getHours() * 60 + now.getMinutes();

    // Safely parse MongoDB String IDs into a number for the math logic
    const numericId = typeof restId === 'string' ? parseInt(restId.slice(-4), 16) || 0 : (restId || 0);

    const schedules = ['11:00 AM - 9:00 PM', '12:00 PM - 10:00 PM', '10:00 AM - 8:00 PM', '4:00 PM - 11:00 PM'];
    const baseTime = dbHours || schedules[numericId % schedules.length];
    ...
    return days.map(dayName => {
      ...
      if (dayName === 'Mon' && numericId % 3 === 0) timeString = 'Closed';
      if (dayName === 'Sun' && numericId % 2 === 0) timeString = 'Closed';
  ...
  useEffect(() => {
    const fetchRestaurantData = async () => {
      try {
        ...
        // 1. Fetch from Restaurant Service (Port 8002)
        const resReq = await fetch(`http://localhost:8002/api/restaurants/${id}`, { headers });
        if (!resReq.ok) throw new Error("Restaurant not found");
        const resData = await resReq.json();
        setRestaurant(resData);

        // 2. Fetch from Review Service (Port 8003)
        try {
          const revReq = await fetch(`http://localhost:8003/api/restaurants/${id}/reviews`, { headers });
          if (revReq.ok) setReviews(await revReq.json());
        } catch (e) { console.error("Review fetch failed", e); }

        // 3. Fetch Photos from Restaurant Service (Port 8002)
        try {
          const photoReq = await fetch(`http://localhost:8002/api/restaurants/${id}/photos`, { headers });
          if (photoReq.ok) setPhotos(await photoReq.json());
  ...
  const handleFavorite = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      // User Service handles favorites
      await fetch(`http://localhost:8001/api/favorites/${id}`, {
        method: 'POST',
  ...
  const handleClaimBusiness = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:8002/api/restaurants/${id}/claim`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
        // THE FIX: We must actually send the owner_id to the backend!
        body: JSON.stringify({ owner_id: user?.id || user?._id }) 
      });
      
      if (!response.ok) {
        const err = await response.json();
        throw new Error(err.detail || "Failed to claim business.");
      }
      
      // Update the local state so the UI instantly changes to the blue "Claimed" badge
      const data = await response.json();
      setRestaurant(prev => ({ ...prev, owner_id: user?.id || user?._id })); 
      setSuccessMsg("Business successfully claimed!");
  ...
  const handleFileChange = async (e) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`http://localhost:8002/api/restaurants/${id}/photos`, {
          method: 'POST',
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:8003/api/restaurants/${id}/reviews`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
      ...
      setReviews([{ ...data, created_at: new Date().toISOString() }, ...reviews]);
      ...
      setRestaurant(prev => ({
        ...
        average_rating: (((prev.average_rating || 0) * (prev.review_count || 0)) + Number(rating)) / ((prev.review_count || 0) + 1)
      }));
      setTimeout(() => setSuccessMsg(''), 3000);
    } catch (error) {
      setSubmitError(error.message);
  ...
  const handleDeleteReview = async (reviewId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await fetch(`http://localhost:8003/api/reviews/${reviewId}`, {
         method: 'DELETE',
  ...
  const handleEditSubmit = async (reviewId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:8003/api/reviews/${reviewId}`, {
         method: 'PUT',
         headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
         body: JSON.stringify({ rating: Number(editRating), comment: editComment })
      });
      const data = await response.json();
      setReviews(reviews.map(r => (r.id || r._id) === reviewId ? { ...r, rating: data.rating, comment: data.comment } : r));
  ...
  const currentHours = getDynamicHours(restaurant.id || restaurant._id);
  const todaySchedule = currentHours.find(h => h.isToday) || currentHours[0];
  
  const unspashFallbackImages = getRestaurantImages(restaurant.cuisine || restaurant.cuisine_type);
      ...
      <div className="relative w-full h-[400px] bg-gray-900 overflow-hidden flex cursor-pointer" onClick={() => setIsGalleryOpen(true)}>
        ...
        <div className="absolute bottom-0 left-0 w-full p-8 max-w-7xl mx-auto flex flex-col justify-end">
          ...
          <div className="flex items-center gap-3 mb-2">
            <div className="flex">
              {renderStars(restaurant.average_rating || 0, "w-8 h-8")}
            </div>
            <span onClick={scrollToReviewsList} className="text-gray-300 font-normal text-lg hover:underline cursor-pointer">({restaurant.review_count || 0} reviews)</span>
          </div>
          ...
          <div className="flex justify-between items-center">
             <div className="flex items-center gap-2 text-white text-lg font-medium">
                ...
                <span className="text-gray-300">•</span>
                <span>{restaurant.cuisine || restaurant.cuisine_type}</span>
              </div>

              <button onClick={(e) => { e.stopPropagation(); setIsGalleryOpen(true); }} className="bg-white/10 hover:bg-white/20 border border-white/30 text-white px-4 py-1.5 rounded-full text-sm font-bold flex items-center gap-1.5 z-10">
              ...
              {reviews.length === 0 ? (
                ...
                <div className="space-y-8 mt-6">
                  {reviews.map((rev, index) => {
                    ...
                    const isOwner = isAuthenticated && user && (user.id || user._id) === rev.user_id; 
                    const isEditing = editingReviewId === revId;

                    return (
                      <div key={revId || index} className="border-b border-gray-200 pb-8 last:border-0 relative">
                        ...
                        <div className="flex items-start justify-between mb-4">
                          ...
                          {isOwner && !isEditing && (
                            <div className="flex gap-3">
                              ...
                              <button onClick={() => handleDeleteReview(revId)} className="text-sm font-bold text-red-600 hover:text-red-800">Delete</button>
                            </div>
                          )}
                        </div>
                        ...
                        {isEditing ? (
                          <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 mt-2">
                            ...
                            <div className="flex gap-2">
                              <button onClick={() => handleEditSubmit(revId)} className="bg-red-600 text-white px-4 py-1.5 rounded text-sm font-bold hover:bg-red-700">Save Changes</button>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-28 bg-white border border-gray-200 rounded-lg shadow-sm p-0 divide-y divide-gray-200">
              ...
              <div className="flex justify-between items-center p-4">
                <span className="font-bold text-gray-900">{restaurant.phone || '(408) 555-0198'}</span>
