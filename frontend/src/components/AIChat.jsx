...
const AIChat = () => {
  ...
  // RUBRIC: Optional Enhancements - Quick Action Buttons
  const quickActions = [
    "Find dinner tonight",
    "Best rated near me",
    "Vegan options"
  ...
  const handleSend = async (textOverride = null) => {
    ...
    try {
      const response = await axiosClient.post('/ai-assistant/chat', {
        message: textToSend,
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: 'Oops! I had a little trouble connecting to my brain. Make sure Ollama is running!', recommendations: [] }
      ]);
    } finally {
      setIsThinking(false);
    }
      ...
      {isOpen ? (
        <div className="bg-white w-96 sm:w-[26rem] rounded-2xl shadow-2xl border border-gray-200 flex flex-col h-[600px] overflow-hidden transition-all duration-300 transform scale-100">
                    ...
                    {msg.recommendations.map((rec, i) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 mb-2 text-xs font-medium">
                          ...
                          {rec.id ? (
                             <span className="text-gray-500">Database Options</span>
                          ) : (
          ...
          <div className="p-4 bg-white border-t border-gray-200 flex flex-col gap-3">
            {/* Quick Action Buttons rendered above the input */}
            {messages.length === 1 && (
